...
from that registry as the feature requires. Stage-specific: for chart work
load `helm-chart-creation` + `helm-bjw-s-chart` + `kubernetes-skill`; for
`containers/` load `container-creation`; for ApplicationSet wiring load
`gitops-workflows`.
